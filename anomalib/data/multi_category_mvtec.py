...
CATEGORY_NAME = ["bottle", "cable", "capsule", "carpet", "grid", "hazelnut", "leather",
                 "metal_nut", "pill", "screw", "tile", "toothbrush", "transistor", "wood", "zipper"]


def make_multi_category_mvtec_dataset(
    path: Path,
    ...
    |---|---------------|-------|---------|---------------|---------------|----------------|---------------------------------------|-------------|
    |   | path          | split | label   | image_path    | category name | category label | mask_path                             | label_index |
    |---|---------------|-------|---------|---------------|---------------|----------------|---------------------------------------|-------------|
    | 0 | datasets/name |  test |  defect |  filename.png | name          | label          | ground_truth/defect/filename_mask.png | 1           |
    |---|---------------|-------|---------|---------------|---------------|----------------|---------------------------------------|-------------|

    Args:
        path (Path): Path to dataset
        category: Names of the MVTec AD category.
    ...
    Example:
        ...
        >>> samples = make_multi_category_mvtec_dataset(path, category=category, split='train', split_ratio=0.1, seed=0)
        >>> samples.head()
           path         split label image_path                           mask_path                   label_index
        0	datasets/MVTec/bottle	train	good	datasets/MVTec/bottle/train/good/000.png	bottle	0	datasets/MVTec/bottle/ground_truth/good/000_ma...	0
        1	datasets/MVTec/bottle	train	good	datasets/MVTec/bottle/train/good/001.png	bottle	0	datasets/MVTec/bottle/ground_truth/good/001_ma...	0
        2	datasets/MVTec/bottle	train	good	datasets/MVTec/bottle/train/good/002.png	bottle	0	datasets/MVTec/bottle/ground_truth/good/002_ma...	0
        3	datasets/MVTec/bottle	train	good	datasets/MVTec/bottle/train/good/003.png	bottle	0	datasets/MVTec/bottle/ground_truth/good/003_ma...	0
        4	datasets/MVTec/bottle	train	good	datasets/MVTec/bottle/train/good/004.png	bottle	0	datasets/MVTec/bottle/ground_truth/good/004_ma...	0

    Returns:
        DataFrame: an output dataframe containing samples for the requested split (ie., train or test)
    """
    if isinstance(category, str):
        samples_list = [(str(path / category),) + filename.parts[-3:] + (category, CATEGORY_NAME.index(category))
                        for filename in path.glob(f"{category}/**/*.png")]
    else:
        samples_list = []
        for category in category:
            samples_list.extend([(str(path / category),) + filename.parts[-3:] + (category, CATEGORY_NAME.index(category))
                                for filename in path.glob(f"{category}/**/*.png")])
    if len(samples_list) == 0:
        raise RuntimeError(f"Found 0 images in {path}")

    samples = pd.DataFrame(samples_list, columns=[
                           "path", "split", "label", "image_path", "category_name", "category_label"])
    ...
    samples["image_path"] = samples.path + "/" + samples.split + \
        "/" + samples.label + "/" + samples.image_path
    ...
    samples.loc[(samples.split == "test") & (
        samples.label == "good"), "mask_path"] = ""
...
class MultiCategoryMVTecDataset(VisionDataset):
    ...
    def __init__(
        ...
        """Multi Category Mvtec AD Dataset class.
        ...
        Args:
            ...
            task: ``category_classification`` or ``anomaly_classification`` or ``segmentation``
            seed: seed used for the random subset splitting
            create_validation_set: Create a validation subset in addition to the train and test subsets

        Examples:
            >>> from anomalib.data.mvtec import MultiCategoryMVTecDataset
            >>> from anomalib.data.transforms import PreProcessor
            >>> pre_process = PreProcessor(image_size=256)
            >>> dataset = MultiCategoryMVTecDataset(
            ...     root='./datasets/MVTec',
            ...     category='leather',
        ...
        self.category: Union[str, List[str]] = category
        self.split = split
        self.task = task

        self.pre_process = pre_process

        assert self.category_is_correct(
            category), 'Please Check the category Config!'

        self.samples = make_multi_category_mvtec_dataset(
            path=self.root,
    ...
    def category_is_correct(self, category) -> bool:
        if isinstance(category, str):
            return category in CATEGORY_NAME
        return all(category in CATEGORY_NAME for category in category)
    ...
    def __getitem__(self, index: int) -> Dict[str, Union[str, Tensor]]:
        ...
        item["category_label"] = self.samples.category_label[index]
...
@DATAMODULE_REGISTRY
class Multi_Category_MVTec(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        """
        logger.info(
            "Setting up train, validation, test and prediction datasets.")
        if stage in (None, "fit"):
            self.train_data = MultiCategoryMVTecDataset(
                root=self.root,
                category=self.category,
        ...
        if self.create_validation_set:
            self.val_data = MultiCategoryMVTecDataset(
                root=self.root,
                category=self.category,
        ...
        self.test_data = MultiCategoryMVTecDataset(
            root=self.root,
            category=self.category,
            pre_process=self.pre_process_val,
            split="test",
