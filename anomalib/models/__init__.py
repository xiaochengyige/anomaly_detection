...
from anomalib.models.multi_category_patchcore import Multi_Category_Patchcore
...
def get_model(config: Union[DictConfig, ListConfig]) -> AnomalyModule:
    ...
    if config.model.name in model_list:
        ...
        model = getattr(
            module, f"{_snake_to_pascal_case(config.model.name)}Lightning")(config)

    else:
        raise ValueError(f"Unknown model {config.model.name}!")

    if "init_weights" in config.keys() and config.init_weights:
        model.load_state_dict(load(os.path.join(config.project.path, config.init_weights))[
